...
const PriceChart = memo(function PriceChart({
  ...
  return (
    <div className='bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-xl border border-slate-200/50 dark:border-slate-700/50 p-4 sm:p-6 lg:p-8 min-h-80'>
      ...
      {loading ? (
        <div
          className='h-[250px] sm:h-[300px] bg-slate-100 dark:bg-slate-700 rounded-xl sm:rounded-2xl animate-pulse flex items-center justify-center'
          role='status'
          aria-live='polite'
          aria-busy='true'
        >
            ...
            <div className='mb-4 sm:mb-6'>
              <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0'>
                <div>
                  ...
                  >
                    {history.data[history.data.length - 1].price.toLocaleString(
                      undefined,
                      { maximumFractionDigits: 2 }
                    )}{" "}
                    {vs}
