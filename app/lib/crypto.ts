...
export function getCryptoLocalizedName(
  crypto: Cryptocurrency,
  locale: string
): string {
  // Try to get name in current locale
  if (crypto.names?.[locale as keyof NonNullable<typeof crypto.names>]) {
    return crypto.names[locale as keyof NonNullable<typeof crypto.names>]!;
  }

  // Fallback to English
  if (crypto.names?.en) {
    return crypto.names.en;
  }

  // Final fallback to crypto name
  return crypto.name;
}
